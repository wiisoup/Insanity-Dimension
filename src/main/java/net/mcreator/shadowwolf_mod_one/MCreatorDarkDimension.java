...
public class MCreatorDarkDimension extends shadowwolf_mod_one.ModElement {

	public static int DIMID = 6;
	public static final boolean NETHER_TYPE = false;
	@GameRegistry.ObjectHolder("shadowwolf_mod_one:darkdimension")
	public static final Item block = null;
	@GameRegistry.ObjectHolder("shadowwolf_mod_one:darkdimension_portal")
	public static final BlockCustomPortal portal = null;
	public static DimensionType dtype;

	public MCreatorDarkDimension(shadowwolf_mod_one instance) {
		super(instance);
		instance.blocks.add(() -> new BlockCustomPortal());
		instance.items.add(() -> new ItemBlock(portal).setRegistryName(portal.getRegistryName()));
		instance.items.add(() -> new ModTrigger().setUnlocalizedName("darkdimension").setRegistryName("darkdimension"));
	}
	...
	public void preInit(FMLPreInitializationEvent event) {
		if (DimensionManager.isDimensionRegistered(DIMID)) {
			...
			System.err.println("Dimension ID for dimension darkdimension is already registered. Falling back to ID: " + DIMID);
		}
		dtype = DimensionType.register("darkdimension", "_darkdimension", DIMID, WorldProviderMod.class, true);
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(block, 0, new ModelResourceLocation("shadowwolf_mod_one:darkdimension", "inventory"));
	}
	...
	public static class WorldProviderMod extends WorldProvider {
		...
		public void onPlayerAdded(EntityPlayerMP entity) {
			...
			int z = (int) entity.posZ;
			{
				...
				MCreatorDarkDimensionAchievementProcedure.executeProcedure($_dependencies);
			}
		}
	}
					...
					for (int k1 = -2; k1 <= 2; ++k1) {
						for (int l1 = -1; l1 < 3; ++l1) {
							...
							this.world.setBlockState(new BlockPos(i2, j2, k2),
									flag ? MCreatorDarkStone.block.getDefaultState() : Blocks.AIR.getDefaultState());
						}
					}
				}
					...
					for (int l7 = 1; l7 < 3; ++l7) {
						for (int k8 = -1; k8 < 3; ++k8) {
							this.world.setBlockState(new BlockPos(i6 + (l7 - 1) * l6 + j7 * i3, k2 + k8, k6 + (l7 - 1) * i3 - j7 * l6), (k8 < 0)
									? MCreatorDarkStone.block.getDefaultState()
				...
				for (int l8 = 0; l8 < 4; ++l8) {
					for (int l9 = -1; l9 < 4; ++l9) {
						this.world.setBlockState(new BlockPos(i6 + (l8 - 1) * l6, k2 + l9, k6 + (l8 - 1) * i3),
								(l8 == 0 || l8 == 3 || l9 == -1 || l9 == 3) ? MCreatorDarkStone.block.getDefaultState() : iblockstate, 2);
					}
				}
	...
	public static class BlockCustomPortal extends BlockPortal {
		...
		public BlockCustomPortal() {
			setHardness(-1.0F);
			setUnlocalizedName("darkdimension_portal");
			setRegistryName("darkdimension_portal");
			setLightLevel(0F);
		}
			...
			if (rand.nextInt(100) == 0)
				world.playSound((double) pos.getX() + 0.5D, (double) pos.getY() + 0.5D, (double) pos.getZ() + 0.5D,
						(net.minecraft.util.SoundEvent) net.minecraft.util.SoundEvent.REGISTRY.getObject(new ResourceLocation(
								("block.furnace.fire_crackle"))), SoundCategory.BLOCKS, 0.5F, rand.nextFloat() * 0.4F + 0.8F, false);
			for (int i = 0; i < 4; ++i) {
		...
		public static class Size {
			...
			protected int getDistanceUntilEdge(BlockPos p_180120_1_, EnumFacing p_180120_2_) {
				...
				for (i = 0; i < 22; ++i) {
					...
					if (!this.isEmptyBlock(this.world.getBlockState(blockpos).getBlock())
							|| this.world.getBlockState(blockpos.down()).getBlock() != MCreatorDarkStone.block.getDefaultState().getBlock()) {
						break;
					}
				}
				Block block = this.world.getBlockState(p_180120_1_.offset(p_180120_2_, i)).getBlock();
				return block == MCreatorDarkStone.block.getDefaultState().getBlock() ? i : 0;
			}
			...
			protected int calculatePortalHeight() {
				label56 : for (this.height = 0; this.height < 21; ++this.height) {
					for (int i = 0; i < this.width; ++i) {
						...
						if (i == 0) {
							...
							if (block != MCreatorDarkStone.block.getDefaultState().getBlock()) {
								break label56;
							}
				...
				for (int j = 0; j < this.width; ++j) {
					if (this.world.getBlockState(this.bottomLeft.offset(this.rightDir, j).up(this.height)).getBlock() != MCreatorDarkStone.block
	...
	public static class ModTrigger extends Item {
		...
		public EnumActionResult onItemUse(EntityPlayer entity, World world, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY,
			...
			if (!entity.canPlayerEdit(pos, facing, itemstack)) {
				...
				{
					java.util.HashMap<String, Object> $_dependencies = new java.util.HashMap<>();
					MCreatorEnterAllDimensions4.executeProcedure($_dependencies);
				}
	...
	public static class ChunkProviderModded implements IChunkGenerator {
		...
		protected static final IBlockState END_STONE = MCreatorDarkStone.block.getDefaultState();
		protected static final IBlockState END_STONE2 = MCreatorDarkDirt.block.getDefaultState();
