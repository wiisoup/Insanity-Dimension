...
public class MCreatorChest12 extends shadowwolf_mod_one.ModElement {

	public MCreatorChest12(shadowwolf_mod_one instance) {
		super(instance);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (dimID == MCreatorDarkDimension.DIMID) {
			...
			isNetherType = MCreatorDarkDimension.NETHER_TYPE;
		}
		if (dimID == MCreatorUno.DIMID) {
			dimensionCriteria = true;
			isNetherType = MCreatorUno.NETHER_TYPE;
		}
		if (dimID == MCreatorPrankdDimension.DIMID) {
			dimensionCriteria = true;
			isNetherType = MCreatorPrankdDimension.NETHER_TYPE;
		}
		...
		if ((random.nextInt(1000000) + 1) <= 2500) {
			...
			Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
					new ResourceLocation("shadowwolf_mod_one", "chest12"));
			if (template == null)
				return;
