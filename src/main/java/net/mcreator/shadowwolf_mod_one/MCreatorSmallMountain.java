...
public class MCreatorSmallMountain extends shadowwolf_mod_one.ModElement {

	public MCreatorSmallMountain(shadowwolf_mod_one instance) {
		super(instance);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if ((random.nextInt(1000000) + 1) <= 2250) {
			...
			int j = Math.abs(random.nextInt(Math.abs(height)) - 24);
			if (world.isRemote)
				return;
