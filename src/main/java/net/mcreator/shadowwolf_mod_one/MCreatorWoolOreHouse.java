...
public class MCreatorWoolOreHouse extends shadowwolf_mod_one.ModElement {

	public MCreatorWoolOreHouse(shadowwolf_mod_one instance) {
		super(instance);
	}
	...
	public void generateWorld(Random random, int i2, int k2, World world, int dimID, IChunkGenerator cg, IChunkProvider cp) {
		...
		if (!dimensionCriteria)
			return;
		if ((random.nextInt(1000000) + 1) <= 1750) {
			...
			Template template = ((WorldServer) world).getStructureTemplateManager().getTemplate(world.getMinecraftServer(),
					new ResourceLocation("shadowwolf_mod_one", "wool"));
			if (template == null)
				return;
